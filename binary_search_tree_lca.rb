=begin
Using the Ruby language, have the function BinarySearchTreeLCA(strArr) 
take the array of strings stored in strArr, which will contain 3 elements: 
the first element will be a binary search tree with all unique values in a preorder 
...
For the input above, your program should return 5 
because that is the value of the node that is the LCA of the two nodes 
...
=end
	...
	# if the two num is under the same node, and one of the two nums is less than 
  #the number before it, then the number before it in the array should be the LCA, 
  #elsif, if the two nums both greater than the number before them, then the LCA 
  #is two numbers to the left. 
	# if the two nums are not under the same node, and is the first two numbers 
  #in the array, then the root number is the LCA
